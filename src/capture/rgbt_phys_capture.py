...
class RGBTPhysCaptureThread(BaseCaptureThread):
    """
    A dedicated thread to capture synchronized RGB, thermal, and physiological data
    using the RGBTPhys_CPP library.

    This class wraps the C++ implementation of RGBTPhys_CPP to provide synchronized
    data capture within the Python framework of the GSR-RGBT project.
    ...
    def __init__(self, config_file=None, base_save_path=None, participant_id=None, 
        ...
        """
        Initializes the RGBTPhys capture thread.

        Args:
            config_file (str): Path to the RGBTPhys_CPP configuration file.
        ...
        """
        super().__init__(device_name="RGBTPhys", parent=parent)
    ...
    def _run_simulation(self):
        ...
        """
        logging.info("Running in RGBTPhys simulation mode.")
        ...
        while self.is_running:
            ...
            logging.info(f"RGBTPhysCaptureThread (simulation): Emitting data with timestamp {current_capture_time}")
    ...
    def _run_real_capture(self):
        """
        Runs the RGBTPhys_CPP executable to capture synchronized data from real hardware.
        """
        logging.info("Starting RGBTPhys_CPP for synchronized data capture.")
        
        # Path to the RGBTPhys_CPP executable
        rgbt_phys_path = os.path.join(os.getcwd(), "third_party", "RGBTPhys_CPP", "RGBTPhys.exe")
        
        # Path to the configuration file
        config_path = os.path.join(os.getcwd(), "third_party", "RGBTPhys_CPP", self.config_file)
        
        # Ensure the base save path exists
        os.makedirs(self.base_save_path, exist_ok=True)
        
        # Command to run RGBTPhys_CPP
        cmd = [
            rgbt_phys_path,
            config_path,
            self.base_save_path,
        ...
        try:
            # Start the RGBTPhys_CPP process
            ...
            if self.process.returncode is not None and self.process.returncode != 0:
                logging.error(f"RGBTPhys_CPP process exited with error code {self.process.returncode}")
            
        except Exception as e:
            logging.error(f"Error running RGBTPhys_CPP: {e}")
    ...
    def _monitor_process_output(self):
        """
        Monitors the output of the RGBTPhys_CPP process and emits signals when new data is available.
        """
        while self.is_monitoring and self.process and self.process.stdout:
            ...
            logging.info(f"RGBTPhys_CPP: {line.strip()}")
            ...
            # the output format of RGBTPhys_CPP
        ...
        logging.info("RGBTPhys_CPP output monitoring stopped.")

    def _cleanup(self):
        """
        Clean up resources when the thread is stopping.
        """
        logging.info("Cleaning up RGBTPhys capture resources.")
        ...
        if self.process and self.process.poll() is None:
            logging.info("Terminating RGBTPhys_CPP process.")
            try:
                ...
                logging.error(f"Error terminating RGBTPhys_CPP process: {e}")
