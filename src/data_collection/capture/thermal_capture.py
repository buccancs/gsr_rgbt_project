...
class ThermalCaptureThread(BaseCaptureThread):
    """
    A dedicated thread to capture thermal video from a FLIR A65 camera.
    ...
    def __init__(self, camera_index: int = 0, fps: int = 30, simulation_mode: bool = False, parent=None):
        ...
        """
        super().__init__(device_name="Thermal", parent=parent)
    ...
    def _run_simulation(self):
        ...
        while self.is_running:
            ...
            logging.info(f"ThermalCaptureThread (simulation): Emitting frame with timestamp {current_capture_time}")
        ...
        try:
            ...
            while self.is_running:
                try:
                    ...
                    current_capture_time = self.get_current_timestamp()
                    logging.info(f"ThermalCaptureThread (real): Emitting frame with timestamp {current_capture_time}")
                    self.frame_captured.emit(color_frame, current_capture_time)
    ...
    def _cleanup(self):
        ...
        if self.camera is not None:
            ...
            if hasattr(self.camera, 'DeInit'):
                self.camera.DeInit()
            del self.camera
            self.camera = None

        if self.system is not None:
            if hasattr(self.system, 'GetCameras'):
                cam_list = self.system.GetCameras()
                cam_list.Clear()
            if hasattr(self.system, 'ReleaseInstance'):
