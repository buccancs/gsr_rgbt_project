...
def main():
    ...
    for fold, (train_idx, test_idx) in enumerate(logo.split(X, y, groups)):
        ...
        X_test_scaled_reshaped = scaler.transform(X_test.reshape(-1, X_test.shape[-1]))
        ...
        model_save_path = (
            config.OUTPUT_DIR / f"models/fold_{fold+1}_subject_{subject_id}.keras"
        )
        ...
        callbacks = [
            ...
            tf.keras.callbacks.TensorBoard(
                log_dir=str(config.OUTPUT_DIR / f"logs/fold_{fold+1}")
            ),
        ]
        ...
        logging.info(f"Evaluating model on subject {subject_id}...")
        test_loss, test_mse = model.evaluate(X_test_scaled, y_test, verbose=0)

        logging.info(
            f"Test Results for subject {subject_id}: MAE Loss = {test_loss:.4f}, MSE = {test_mse:.4f}"
        )
        fold_results.append({"subject": subject_id, "mae": test_loss, "mse": test_mse})
    ...
    logging.info(
        f"Cross-validation results saved to {config.OUTPUT_DIR / 'cross_validation_results.csv'}"
    )
