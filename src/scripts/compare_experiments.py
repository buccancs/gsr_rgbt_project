...
def load_all_cv_results(base_output_dir: Path, experiment_pattern: str = "cross_validation_results_*.csv") -> Optional[
    ...
    for cv_file in base_output_dir.glob(f"**/{experiment_pattern}"):
        try:
            # First, check if the file is in a structured experiment directory
            # Format: outputs/experiments/exp001_lstm_tuning/cross_validation_results_lstm.csv
            # would become exp001_lstm_tuning_lstm
            if "experiments" in cv_file.parts:
                ...
                if exp_idx + 1 < len(cv_file.parts):  # Make sure there's a directory after "experiments"
                    ...
                    # Fallback to just the filename if the path structure is unexpected
                    match = re.search(r"cross_validation_results_(.+)\.csv", cv_file.name)
                    if match:
                        model_run_id = match.group(1)
                    else:
                        model_run_id = cv_file.stem.replace("cross_validation_results_", "")
            else:
