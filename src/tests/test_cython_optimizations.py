...
class TestCythonOptimizations(unittest.TestCase):
    ...
    def test_cy_align_signals_edge_cases(self):
        ...
        # Extract the data needed for the Cython implementation
        gsr_data = empty_gsr_df.drop(columns=["timestamp"]).values if not empty_gsr_df.empty else np.array([]).reshape(0, 1)
        video_data = empty_video_df.drop(columns=["timestamp"]).values if not empty_video_df.empty else np.array([]).reshape(0, 1)
        gsr_timestamps = empty_gsr_df["timestamp"].astype(np.int64).values if not empty_gsr_df.empty else np.array([])
        video_timestamps = empty_video_df["timestamp"].astype(np.int64).values if not empty_video_df.empty else np.array([])
        
        # Call the Cython implementation directly
        if not empty_gsr_df.empty and not empty_video_df.empty:
            cy_aligned_data = cy_align_signals(gsr_data, video_data, gsr_timestamps, video_timestamps)
            
            # Convert the Cython result to a DataFrame for comparison
            cy_aligned_df = pd.DataFrame(
                cy_aligned_data,
                columns=list(empty_gsr_df.columns[1:]) + list(empty_video_df.columns[1:])
            )
            cy_aligned_df["timestamp"] = empty_gsr_df["timestamp"].reset_index(drop=True)
            
            # Check that the results are the same
            self.assertEqual(len(py_aligned_df), len(cy_aligned_df))
        else:
            # If either DataFrame is empty, the Python implementation should return an empty DataFrame
            self.assertTrue(py_aligned_df.empty)
